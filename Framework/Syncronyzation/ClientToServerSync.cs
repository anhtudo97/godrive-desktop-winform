...
namespace VirtualFileSystem.Syncronyzation
{
    ...
    internal class ClientToServerSync
    {
        /// <summary>
        /// Creates instance of this class.
        /// </summary>
        /// <param name="log">Logger.</param>
        internal ClientToServerSync()
        {
        ...
        internal async Task SyncronizeMovedAsync(string userFileSystemFolderPath)
        {
            ...
            foreach (string userFileSystemPath in userFileSystemChildren)
            {
                ...
                try
                {
                    ...
                    if (!FsPath.AvoidSync(userFileSystemPath))
                    {
                        ...
                        if (userFileSystemItem.IsMoved())
                        {
                            ...
                            await new RemoteStorageRawItem(userFileSystemOldPath).MoveToAsync(userFileSystemPath);
                        }
                        else
                        {
                            ...
                            if (!userFileSystemItem.IsNew() && string.IsNullOrEmpty(userFileSystemOldPath))
                            {
                                ...
                                Debug.WriteLine("Saving Original Path", userFileSystemItem.Path);
                ...
                {
                    Debug.WriteLine("Move in remote storage failed", userFileSystemOldPath, userFileSystemPath, ex);
                }
                ...
                {
                    Debug.WriteLine("Folder move sync failed", userFileSystemPath, null, ex);
                }
            }
        }
...
//            LogMessage("Synchronizing:", userFileSystemFolderPath);
            ...
            foreach (string userFileSystemPath in userFileSystemChildren)
            {
                try
                {
                    if (!FsPath.AvoidSync(userFileSystemPath))
                    {
                        if (PlaceholderItem.GetItem(userFileSystemPath).IsNew())
                        {
                            ...
                            await RemoteStorageRawItem.CreateAsync(userFileSystemPath);
                        }
                        else
                        {
                            // Update file/folder in the remote storage. Unlock if auto-locked.
                            await new RemoteStorageRawItem(userFileSystemPath).UpdateAsync();
                        }
                    }
                }
                catch (Exception ex)
                {
                    Debug.WriteLine("Update failed", userFileSystemPath, null, ex);
                }
                ...
                {
                    Debug.WriteLine("Folder sync failed:", userFileSystemPath, null, ex);
                }
            }
        }
    }
