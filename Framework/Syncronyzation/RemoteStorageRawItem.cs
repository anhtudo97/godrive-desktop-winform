...
namespace VirtualFileSystem.Syncronyzation
{
    ...
    public class RemoteStorageRawItem
    {
        ...
        internal RemoteStorageRawItem(string userFileSystemPath)
        {
        ...
        public static async Task CreateAsync(string userFileSystemNewItemPath)
        {
            try
            {
                Debug.WriteLine("Creating item in the remote storage", userFileSystemNewItemPath);
                await new RemoteStorageRawItem(userFileSystemNewItemPath).CreateOrUpdateAsync(FileMode.CreateNew);
                await new UserFileSystemRawItem(userFileSystemNewItemPath).ClearStateAsync();
                Debug.WriteLine("Created in the remote storage succesefully", userFileSystemNewItemPath);
            }
            catch (Exception ex)
            {
        ...
        public async Task UpdateAsync()
        {
            ...
            try
            {
                if (!PlaceholderItem.GetItem(userFileSystemPath).GetInSync())
                {
                    ...
                    Debug.WriteLine("Sending to remote storage", userFileSystemPath);
                    await CreateOrUpdateAsync(FileMode.Open, lockInfo);
                    //await new UserFileSystemRawItem(userFileSystemPath).ClearStateAsync();
                    Debug.WriteLine("Sent to remote storage succesefully", userFileSystemPath);
                }
                ...
                if (await Lock.GetLockModeAsync(userFileSystemPath) == LockMode.Auto)
                {
                    ...
                    {
                        // Get existing lock.
                        fileLock = await Lock.LockAsync(userFileSystemPath, FileMode.Open, LockMode.None);
                    }
            ...
            catch(ClientLockFailedException ex)
            {
                ...
                Debug.WriteLine(ex.Message, userFileSystemPath);
            }
            catch (Exception ex)
            {
        ...
        internal async Task MoveToAsync(string userFileSystemNewPath, IConfirmationResultContext resultContext = null)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        Debug.WriteLine("Moving item in remote storage", userFileSystemOldPath, userFileSystemNewPath);
                        ...
                        Debug.WriteLine("Moved succesefully in remote storage", userFileSystemOldPath, userFileSystemNewPath);
                    }
                }
                finally
                {
        ...
        {
            // Get existing lock or create a new lock.
            Lock fileLock = await Lock.LockAsync(userFileSystemPath, lockFileOpenMode, lockMode);

            if (fileLock.IsNew())
            {
                Debug.WriteLine("Locking", userFileSystemPath);
                ...
                Debug.WriteLine($"Locked succesefully. Mode: {lockMode}", userFileSystemPath);
            }
        ...
        {
            Debug.WriteLine("Unlocking", userFileSystemPath);
            ...
            Debug.WriteLine("Unlocked succesefully", userFileSystemPath);
        }
    }
}