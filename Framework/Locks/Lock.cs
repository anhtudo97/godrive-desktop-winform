...
namespace VirtualFileSystem
{
    ...
    internal class Lock : IDisposable
    {
        ...
        private Lock(string userFileSystemPath, FileStream lockTokenFileStream)
        {
        ...
        internal static async Task SetLockModeAsync(string userFileSystemPath, LockMode lockMode)
        {
            ...
            FileStream fileStream = File.Open(lockModeFilePath, FileMode.OpenOrCreate, FileAccess.Write, FileShare.Delete);
        ...
        internal static async Task<LockMode> GetLockModeAsync(string userFileSystemPath)
        {
            ...
            try
            {
                FileStream fileStream = File.Open(lockModeFilePath, FileMode.Open, FileAccess.Read, FileShare.Delete);
        ...
        internal static async Task<Lock> LockAsync(string userFileSystemPath, FileMode lockFileOpenMode, LockMode lockMode)
        {
            ...
            try
            {
                lockInfo = new Lock(userFileSystemPath, lockTokenFileStream);
                if (lockInfo.IsNew())
                {
        ...
        private static string GetLockModeFilePath(string userFileSystemPath)
        {
            ...
            string relativePath = Helper.TrimEndingDirectorySeparator(userFileSystemPath).Substring(
                Helper.TrimEndingDirectorySeparator(PersonalProvider.GetRootPath()).Length);

            string path = $"{Helper.TrimEndingDirectorySeparator("")}{relativePath}.lockmode";
            return path;
        }
        ...
        private static string GetLockTokenFilePath(string userFileSystemPath)
        {
            ...
            string path = $"{Helper.TrimEndingDirectorySeparator("")}{relativePath}.locktoken";
            return path;
        }
        ...
        internal void Unlock()
        {
            ...
            {
                Debug.WriteLine("Failed to delete lock-mode file.", userFileSystemPath, null, ex);
            }
            ...
            {
                Debug.WriteLine("Failed to delete lock-token file.", userFileSystemPath, null, ex);
            }
        }
